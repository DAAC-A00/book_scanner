...
- **작업자:** “수천 권을 스캔할 때, 점검을 시작한 뒤에는 버튼 없이 바코드만 갖다 대도 자동으로 인식·기록되어야 한다.”
- **작업자:** “앱에 들어오자마자 카메라가 켜지지 않고, **「장서점검 시작」**을 눌렀을 때만 권한·프리뷰가 열렸으면 한다.”
- **작업자:** “한 번 시작한 점검에서 찍은 코드는 **시작 시각을 기준으로 한 저장소 키**에만 줄바꿈으로 쌓이고, **스캔할 때마다 바로** 저장되어야 한다. 중단 시점에 한꺼번에 저장되면 안 된다.”
- **작업자:** “점검을 시작하기 전에 **예전에 저장해 둔 점검 기록**을 목록에서 고르고, 내용을 보거나 고치거나 지울 수 있어야 한다.”
- **작업자:** “QR·문자가 섞인 코드는 장서 번호가 아니므로 **숫자만** 받아들이고, 화면에서 **방금 인식된 값**을 바로 알 수 있어야 한다.”
- **관리자:** “서고처럼 네트워크가 불안정해도 스캔 데이터가 **브라우저 로컬에 보존**되고, 이후 보내기·동기화 전략을 붙일 수 있어야 한다.”
...
- **세션 종료:** **「점검 중단」**으로 `endInventorySession()` — 카메라·인스턴스 정리. 이미 스캔분은 스캔 시점마다 저장되어 있으므로 **종료 시 일괄 저장 없음**.
